...
class LRHRlmdbDatasetwithcoeff(data.Dataset):
    '''
    Read LR and HR image pair.
    If only HR image is provided, generate LR image on-the-fly.
    The pair is ensured by 'sorted' function, so please check the name convention.
    ...
    def name(self):
        return 'LRHRlmdbDatasetwithcoeff'

    def __init__(self, opt):
        super(LRHRlmdbDatasetwithcoeff, self).__init__()
        self.opt = opt
        self.paths_LR = None
        self.paths_HR = None
        self.paths_coeff = None
        self.LR_env = None  # environment for lmdb
        self.HR_env = None
        self.coeff_env = None
        ...
        self.HR_env, self.paths_HR = common.get_image_paths(opt['data_type'], opt['dataroot_HR'])
        self.LR_env, self.paths_LR = common.get_image_paths(opt['data_type'], opt['dataroot_LR'])
        self.coeff_env, self.path_coeff = common.get_image_paths(opt['data_type'], opt['coeffroot_LMDB'])
        assert self.paths_HR, 'Error: HR paths are empty.'
        if self.paths_LR and self.paths_HR:
            assert len(self.paths_LR) == len(self.paths_HR), \
                'HR and LR datasets have different number of images - {}, {}.'.format( \
